...
6. Multi-Split-K Support (Iterate over different split ratios).
"""
import os
import sys
import torch
...
def parse_args():
    ...
    # Strategy Params
    # Modified: Support multiple split_k values
    parser.add_argument('--split_k', type=float, nargs='+', default=[0.6],
                        help='Front K% layers (can be multiple, e.g. 0.5 0.6 0.8)')

    # Multi-Rank Support: Pass groups of 3 integers
    # e.g. --ranks 32 -1 64  (One config)
    # e.g. --ranks 32 -1 64 16 -1 32 (Two configs)
    parser.add_argument('--ranks', type=int, nargs='+', default=[32, -1, 32],
...
def visualize_results(results_df, rank_allocations, output_dir):
    ...
    # Plot 1: Rank Allocation (Line Plot)
...
def main():
    ...
    for idx, (base_r, front_r, back_r) in enumerate(rank_configs):
        ...
        # Optimized (Split) - Iterate over all split_k values
        for k_val in args.split_k:
            opt_name = f"Split_r{front_r}-{back_r}_K{k_val}_Set{idx + 1}"
            alloc, active = calculate_rank_allocation(num_layers, k_val, front_r, back_r)
            strategies[opt_name] = {
                'type': 'lora',
                'allocation': alloc,
